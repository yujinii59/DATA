...
, TB_SALES_CUST_ITEM AS (
 ...
 SELECT PLAN_EDT.PROJECT_CD
	  ...
	  , PLAN_EDT.PLAN_WEEK
	  , ISNULL(EVN.EVN_PRICE,ISNULL(PRICE.CON_PRICE,0)) CON_PRICE
	  , ISNULL(EVN.EVN_PRICE,ISNULL(PRICE.FAC_PRICE,0)) FAC_PRICE
	  , ISNULL(EVN.EVN_PRICE,ISNULL(PRICE.SHP_PRICE,0)) SHP_PRICE
	  , ISNULL(EVN.EVN_PRICE,ISNULL(PRICE.AGN_PRICE,0)) AGN_PRICE
	  , 0					  AS ATTR02
	  , CASE WHEN :VS_PRICE = 35
	         THEN ISNULL(EVN.EVN_PRICE,ISNULL(PRICE.CON_PRICE,0))
	         WHEN :VS_PRICE = 36
	         THEN ISNULL(EVN.EVN_PRICE,ISNULL(PRICE.FAC_PRICE,0))
	         WHEN :VS_PRICE = 37
	         THEN ISNULL(EVN.EVN_PRICE,ISNULL(PRICE.SHP_PRICE,0))
	         WHEN :VS_PRICE = 38
	         THEN ISNULL(EVN.EVN_PRICE,ISNULL(PRICE.AGN_PRICE,0)) END AS ATTR03
	  , ''					  AS ATTR04
	  , ISNULL(RST.RST_QTY,0) AS ATTR05
	  , 0					  AS ATTR06
	  , ISNULL(FCST.RESULT_SALES,0)	  AS ATTR07
	  , 0				AS ATTR08
	  , 0				AS ATTR09
	  , 0				AS ATTR10
	  , 0				AS ATTR11
	...
	LEFT HASH JOIN (
			...
			FROM M4S_O110600  FCST
				...
				AND FCST.ITEM_CD = SC.ITEM_CD
				 INNER JOIN M4S_O201010 SPV
				 ON SPV.DP_VRSN_ID = 'SP_2021W49.01'
				 AND FCST.DATA_VRSN_CD = SPV.DF_VRSN_ID
